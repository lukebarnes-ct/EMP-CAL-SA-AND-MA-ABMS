...
wMax_Chart = 75       # Max Exponential Moving Average Periods
wMin_Chart = 15       # Min Exponential Moving Average Periods
...
printOutput(plotStart, plotEnd, 5, "ER")
printOutput(plotStart, plotEnd, 5, "Prop")
printOutput(plotStart, plotEnd, 5, "Invest")
printOutput(plotStart, plotEnd, 5, "Wealth")
printOutput(plotStart, plotEnd, 5, "Price")
...
getMoments(returns, plotStart, plotEnd, "Simulated", "JSE", "Weekly")
getMoments(returnsJSE_Weekly, 1, lengthJSE_Weekly, "Real", "JSE", "Weekly")

#################################################################################

# Price Plots

function plotPrices(Prices, empPrices, bt, et, kF, kC)

    t = bt:et

    wt = length(empPrices)

    jse = plot(1:wt, empPrices, label = "JSE Top 40", title = "JSE Top 40 Index", 
    ...
    plot(p1, p2, p3, p4, p5, jse, layout = (3, 2), size = (900, 900), 
         margin = 2mm)

end

display(plotPrices(prices, weekly_JSETOP40_Data, plotStart, plotEnd, numFund, numChart))

#################################################################################

# Asset Return Plots

function plotReturns(Returns, empReturns, bt, et, kF, kC)

    t = bt:et

    wt = length(empReturns)

    jse = plot(1:wt, empReturns, title = "JSE Top 40 Index", label = false, 
    ...
    hline!([mean(empReturns)], label = round(mean(empReturns), digits = 4), 
            color =:black, lw = 1, linestyle =:dash)

    p1 = plot(t, Returns[1, t], label = false, title = "Asset 1", 
              xlabel = "Week", ylabel = "Returns", legend = :topleft, framestyle = :box, 
              tick_direction = :none, color = "darkorange2", ylim = (-0.15, 0.25), 
              grid = (:y, :auto), gridlinewidth = 1.5, gridalpha = 0.125)

        
    hline!([mean(Returns[1, t])], label = round(mean(Returns[1, t]), digits = 4), 
            color =:black, lw = 1, linestyle =:dash)

    p2 = plot(t, Returns[2, t], label = false, title = "Asset 2", 
                xlabel = "Week", ylabel = "Returns", legend = :topleft, framestyle = :box, 
                tick_direction = :none, color = "darkorange2", ylim = (-0.15, 0.25), 
                grid = (:y, :auto), gridlinewidth = 1.5, gridalpha = 0.125)
                
        
    hline!([mean(Returns[2, t])], label = round(mean(Returns[2, t]), digits = 4), 
            color =:black, lw = 1, linestyle =:dash)

    p3 = plot(t, Returns[3, t], label = false, title = "Asset 3", 
                xlabel = "Week", ylabel = "Returns", legend = :topleft, framestyle = :box, 
                tick_direction = :none, color = "darkorange2", ylim = (-0.15, 0.25), 
                grid = (:y, :auto), gridlinewidth = 1.5, gridalpha = 0.125)

        
    hline!([mean(Returns[3, t])], label = round(mean(Returns[3, t]), digits = 4), 
            color =:black, lw = 1, linestyle =:dash)

    p4 = plot(t, Returns[4, t], label = false, title = "Asset 4", 
                xlabel = "Week", ylabel = "Returns", legend = :topleft, framestyle = :box, 
                tick_direction = :none, color = "darkorange2", ylim = (-0.15, 0.25), 
                grid = (:y, :auto), gridlinewidth = 1.5, gridalpha = 0.125)

        
    hline!([mean(Returns[4, t])], label = round(mean(Returns[4, t]), digits = 4), 
            color =:black, lw = 1, linestyle =:dash)

    p5 = plot(t, Returns[5, t], label = false, title = "Asset 5", 
                xlabel = "Week", ylabel = "Returns", legend = :topleft, framestyle = :box, 
                tick_direction = :none, color = "darkorange2", ylim = (-0.15, 0.25), 
                grid = (:y, :auto), gridlinewidth = 1.5, gridalpha = 0.125)

    hline!([mean(Returns[5, t])], label = round(mean(Returns[5, t]), digits = 4), 
            color =:black, lw = 1, linestyle =:dash)
...
display(plotReturns(returns, returnsJSE_Weekly, plotStart, plotEnd, numFund, numChart))

#################################################################################

# Return Distribution Histogram and Density Plots

function plotReturnDistribution(Returns, empReturns, bt, et)
    ...
    jse = histogram(empReturns, bins = binSize, title = "JSE Top 40 Index",
    ...
    jseDensity = pdf.(Normal(mean(empReturns), std(empReturns)), xVals)

    plot!(xVals, jseDensity, label = false, lw = 2.5, color = "purple1")

    p1 = histogram(Returns[1, t], bins = binSize, title = "Asset 1", 
    xlabel = "Returns", ylabel = "Density", xlim = (-0.15, 0.15), normalize = :pdf,
    label = false, color = "darkorange2", alpha = 0.5, framestyle = :box, 
    tick_direction = :none, grid = (:y, :auto), gridlinewidth = 1.5, 
    gridalpha = 0.125)

    a1Density = pdf.(Normal(mean(Returns[1, t]), std(Returns[1, t])), xVals)
    plot!(xVals, a1Density, label = false, lw = 2.5, color = "darkorange2")

    p2 = histogram(Returns[2, t], bins = binSize, title = "Asset 2", 
    xlabel = "Returns", ylabel = "Density", xlim = (-0.15, 0.15), normalize = :pdf,
    label = false, color = "darkorange2", alpha = 0.5, framestyle = :box, 
    tick_direction = :none, grid = (:y, :auto), gridlinewidth = 1.5, 
    gridalpha = 0.125)

    a2Density = pdf.(Normal(mean(Returns[2, t]), std(Returns[2, t])), xVals)
    plot!(xVals, a2Density, label = false, lw = 2.5, color = "darkorange2")

    p3 = histogram(Returns[3, t], bins = binSize, title = "Asset 3", 
    xlabel = "Returns", ylabel = "Density", xlim = (-0.15, 0.15), normalize = :pdf,
    label = false, color = "darkorange2", alpha = 0.5, framestyle = :box, 
    tick_direction = :none, grid = (:y, :auto), gridlinewidth = 1.5, 
    gridalpha = 0.125)

    a3Density = pdf.(Normal(mean(Returns[3, t]), std(Returns[3, t])), xVals)
    plot!(xVals, a3Density, label = false, lw = 2.5, color = "darkorange2")

    p4 = histogram(Returns[4, t], bins = binSize, title = "Asset 4", 
    xlabel = "Returns", ylabel = "Density", xlim = (-0.15, 0.15), normalize = :pdf,
    label = false, color = "darkorange2", alpha = 0.5, framestyle = :box, 
    tick_direction = :none, grid = (:y, :auto), gridlinewidth = 1.5, 
    gridalpha = 0.125)

    a4Density = pdf.(Normal(mean(Returns[4, t]), std(Returns[4, t])), xVals)
    plot!(xVals, a4Density, label = false, lw = 2.5, color = "darkorange2")

    p5 = histogram(Returns[5, t], bins = binSize, title = "Asset 5", 
    xlabel = "Returns", ylabel = "Density", xlim = (-0.15, 0.15), normalize = :pdf,
    label = false, color = "darkorange2", alpha = 0.5, framestyle = :box, 
    tick_direction = :none, grid = (:y, :auto), gridlinewidth = 1.5, 
    gridalpha = 0.125)

    a5Density = pdf.(Normal(mean(Returns[5, t]), std(Returns[5, t])), xVals)
    plot!(xVals, a5Density, label = false, lw = 2.5, color = "darkorange2")
...
display(plotReturnDistribution(returns, returnsJSE_Weekly, plotStart, plotEnd))

#################################################################################

# Return AutoCorrelation Plots

function plotAutoCorrelations(Returns, empReturns, bt, et)
    ...
    sqCol = "orchid"
    # fg = range(0.3, stop = 0.01, length = 25)

    jseRetAuto = autocor(empReturns)[1:(maxLags+1)]
    jseAbsRetAuto = autocor(abs.(empReturns))[1:(maxLags+1)]
    ...
    a1RetAuto = autocor(Returns[1, t])[1:(maxLags+1)]
    a1AbsRetAuto = (autocor(abs.(Returns[1, t]))[1:(maxLags+1)]) # .+ vcat(0, fg)

    p1 = plot(xVals, a1RetAuto, seriestype = :line, title = "Asset 1",
    xlabel = "Lag", ylabel = "Autocorrelation", framestyle = :box, 
    tick_direction = :none, color = "darkorange2", lw = 1.5, 
    gridlinewidth = 1.5, gridstyle = :dash, gridalpha = 0.125, label = false)
    ...
    a2RetAuto = autocor(Returns[2, t])[1:(maxLags+1)]
    a2AbsRetAuto = autocor(abs.(Returns[2, t]))[1:(maxLags+1)] # .+ vcat(0, fg)

    p2 = plot(xVals, a2RetAuto, seriestype = :line, title = "Asset 2",
    xlabel = "Lag", ylabel = "Autocorrelation", framestyle = :box, 
    tick_direction = :none, color = "darkorange2", lw = 1.5, 
    gridlinewidth = 1.5, gridstyle = :dash, gridalpha = 0.125, label = false)

    scatter!(xVals, a2RetAuto, marker = (:circle, 4, stroke(:darkorange2)), 
    color = "darkorange2", label = "Returns")

    hline!([0], label = false, color =:black, lw = 0.75)

    plot!(xVals, a2AbsRetAuto, seriestype = :line, label = false, 
    color = sqCol, lw = 1.5)

    scatter!(xVals, a2AbsRetAuto, marker = (:square, 4, stroke(sqCol)), 
    color = sqCol, label = "|Returns|")

    a3RetAuto = autocor(Returns[3, t])[1:(maxLags+1)]
    a3AbsRetAuto = autocor(abs.(Returns[3, t]))[1:(maxLags+1)] # .+ vcat(0, fg)

    p3 = plot(xVals, a3RetAuto, seriestype = :line, title = "Asset 3",
    xlabel = "Lag", ylabel = "Autocorrelation", framestyle = :box, 
    tick_direction = :none, color = "darkorange2", lw = 1.5, 
    gridlinewidth = 1.5, gridstyle = :dash, gridalpha = 0.125, label = false)

    scatter!(xVals, a3RetAuto, marker = (:circle, 4, stroke(:darkorange2)), 
    color = "darkorange2", label = "Returns")

    hline!([0], label = false, color =:black, lw = 0.75)

    plot!(xVals, a3AbsRetAuto, seriestype = :line, label = false, 
    color = sqCol, lw = 1.5)

    scatter!(xVals, a3AbsRetAuto, marker = (:square, 4, stroke(sqCol)), 
    color = sqCol, label = "|Returns|")

    a4RetAuto = autocor(Returns[4, t])[1:(maxLags+1)]
    a4AbsRetAuto = autocor(abs.(Returns[4, t]))[1:(maxLags+1)] # .+ vcat(0, fg)

    p4 = plot(xVals, a4RetAuto, seriestype = :line, title = "Asset 4",
    xlabel = "Lag", ylabel = "Autocorrelation", framestyle = :box, 
    tick_direction = :none, color = "darkorange2", lw = 1.5, 
    gridlinewidth = 1.5, gridstyle = :dash, gridalpha = 0.125, label = false)

    scatter!(xVals, a4RetAuto, marker = (:circle, 4, stroke(:darkorange2)), 
    color = "darkorange2", label = "Returns")

    hline!([0], label = false, color =:black, lw = 0.75)

    plot!(xVals, a4AbsRetAuto, seriestype = :line, label = false, 
    color = sqCol, lw = 1.5)

    scatter!(xVals, a4AbsRetAuto, marker = (:square, 4, stroke(sqCol)), 
    color = sqCol, label = "|Returns|")

    a5RetAuto = autocor(Returns[5, t])[1:(maxLags+1)]
    a5AbsRetAuto = autocor(abs.(Returns[5, t]))[1:(maxLags+1)] # .+ vcat(0, fg)

    p5 = plot(xVals, a5RetAuto, seriestype = :line, title = "Asset 5",
    xlabel = "Lag", ylabel = "Autocorrelation", framestyle = :box, 
    tick_direction = :none, color = "darkorange2", lw = 1.5, 
    gridlinewidth = 1.5, gridstyle = :dash, gridalpha = 0.125, label = false)

    scatter!(xVals, a5RetAuto, marker = (:circle, 4, stroke(:darkorange2)), 
    color = "darkorange2", label = "Returns")

    hline!([0], label = false, color =:black, lw = 0.75)

    plot!(xVals, a5AbsRetAuto, seriestype = :line, label = false, 
    color = sqCol, lw = 1.5)

    scatter!(xVals, a5AbsRetAuto, marker = (:square, 4, stroke(sqCol)), 
    color = sqCol, label = "|Returns|")
...
display(plotAutoCorrelations(returns, returnsJSE_Weekly, plotStart, plotEnd))
