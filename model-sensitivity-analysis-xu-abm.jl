...
optVal_XU_BSESN_Weekly = output__BSESN_Weekly[argmin(output__BSESN_Weekly.Objective), 5]

#####################################################################

minWF_fix = bestParameters_BSESN_Weekly[2]
minWC_fix = bestParameters_BSESN_Weekly[4]

output__BSESN_Weekly_fixed = filter(row -> row.MinWindFund == minWF_fix && row.MinWindChart == minWC_fix, output__BSESN_Weekly)

xvals = unique(output__BSESN_Weekly_fixed.MaxWindFund)
yvals = unique(output__BSESN_Weekly_fixed.MaxWindChart)

output_maxWF_maxWX_pivot = unstack(output__BSESN_Weekly_fixed, :MaxWindFund, :MaxWindChart, :Objective)
zvals = Matrix(select(output_maxWF_maxWX_pivot, Not(:MaxWindFund)))

figXU = Figure(figure_padding = 40)
axisXU = Axis3(figXU[1, 1], xlabel = L"τ^F_{max}", ylabel = L"τ^C_{max}", zlabel = "")
surfXU = GLMakie.surface!(axisXU, xvals, yvals, 
                          zvals, 
                          colormap =:plasma, label = "f(0)", colorscale = log10)

Colorbar(figXU[1, 2], surfXU)
GLMakie.scatter!(axisXU, [bestParameters_BSESN_Weekly[1]], [bestParameters_BSESN_Weekly[3]], [optVal_XU_BSESN_Weekly + 5], 
                 color =:red, markersize = 10, strokewidth = 2, strokecolor =:black)
figXU

save("Plots/xu-calibration/sensitivity-analysis/bsesn_weekly_tau_fmax_tau_cmax.png", 
     figXU, px_per_unit = 4)

#####################################################################

maxWF_fix = bestParameters_BSESN_Weekly[1]
maxWC_fix = bestParameters_BSESN_Weekly[3]

output__BSESN_Weekly_fixed_max = filter(row -> row.MaxWindFund == maxWF_fix && row.MaxWindChart == maxWC_fix, output__BSESN_Weekly)

xvals_min = unique(output__BSESN_Weekly_fixed_max.MinWindFund)
yvals_min = unique(output__BSESN_Weekly_fixed_max.MinWindChart)

output_minWF_minWX_pivot = unstack(output__BSESN_Weekly_fixed_max, :MinWindFund, :MinWindChart, :Objective)
zvals_min = Matrix(select(output_minWF_minWX_pivot, Not(:MinWindFund)))

figXU_min = Figure(figure_padding = 40)
axisXU_min = Axis3(figXU_min[1, 1], xlabel = L"τ^F_{min}", ylabel = L"τ^C_{min}", zlabel = "")
surfXU_min = GLMakie.surface!(axisXU_min, xvals_min, yvals_min, 
                          zvals_min, 
                          colormap =:plasma, label = "f(0)", colorscale = log10)

Colorbar(figXU_min[1, 2], surfXU_min)
GLMakie.scatter!(axisXU_min, [bestParameters_BSESN_Weekly[2]], [bestParameters_BSESN_Weekly[4]], [optVal_XU_BSESN_Weekly + 5], 
                 color =:red, markersize = 10, strokewidth = 2, strokecolor =:black)
figXU_min
save("Plots/xu-calibration/sensitivity-analysis/bsesn_weekly_tau_fmin_tau_cmin.png", 
     figXU_min, px_per_unit = 4)
