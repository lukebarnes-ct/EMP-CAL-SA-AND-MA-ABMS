...
function descriptiveStatistics(Returns, empReturns, bt, et)
    ...
    a2Returns = Returns[2, t]
    a3Returns = Returns[3, t]
    a4Returns = Returns[4, t]
    a5Returns = Returns[5, t]

    a1Mean = round(mean(a1Returns), digits = 4)
    a2Mean = round(mean(a2Returns), digits = 4)
    a3Mean = round(mean(a3Returns), digits = 4)
    a4Mean = round(mean(a4Returns), digits = 4)
    a5Mean = round(mean(a5Returns), digits = 4)
    jseMean = round(mean(returnsJSE), digits = 4)

    a1Median = round(median(a1Returns), digits = 4)
    a2Median = round(median(a2Returns), digits = 4)
    ...
    descStat = Table(Asset = ["1", "2", "3", "4", "5", "JSE"], 
                     ...
                     Median = [a1Median, a2Median, a3Median, a4Median, a5Median, jseMedian],
                     SD = [a1SD, a2SD, a3SD, a4SD, a5SD, jseSD],
                     Skewness = [a1Skewness, a2Skewness, a3Skewness, a4Skewness, a5Skewness, jseSkewness],
                     Kurtosis = [a1Kurtosis, a2Kurtosis, a3Kurtosis, a4Kurtosis, a5Kurtosis, jseKurtosis])
...
function plotPrices(Prices, bt, et, kF, kC)
...
display(plotPrices(prices, plotStart, plotEnd, numFund, numChart))
...
function plotReturns(Returns, bt, et, kF, kC)
    ...
    jse = plot(1:wt, jRet[1, :], title = "JSE Top 40 Index", label = false, 
    ...
    hline!([mean(jRet)], label = round(mean(jRet), digits = 4), 
...
display(plotReturns(returns, plotStart, plotEnd, numFund, numChart))
...
@save "Data/XU_Calibration/FINAL_Prices.jld2" prices
@save "Data/XU_Calibration/FINAL_Returns.jld2" returns
